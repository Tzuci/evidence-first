"""Integration tests for the Phase 8.8A claim entailment step inside
apps/worker/app/consumers/task_created.py.

Coverage map (4 scenarios required by the block prompt §5):

  1. test_pipeline_emits_task_entailment_checked_after_source_quality_before_compiling
  2. test_pipeline_populates_claim_entailment_checks_for_claim_evidence_pairs
  3. test_pipeline_resume_from_compiling_does_not_reemit_entailment_audit
  4. test_pipeline_entailment_failure_is_audited_but_does_not_block_8_4

Design notes:

  - These are DB-real tests. They drive the full task.created
    pipeline against the real Postgres so the 8.8A step is exercised
    in the context of all the other steps (extractor, CVE-lite,
    source quality, compiler, gate). No Redis, no API, no
    dispatcher: we call ``handle_task_created`` directly with a
    decoded event dict, mirroring the pattern used by
    apps/worker/tests/test_task_created_source_quality_step.py.

  - Helpers in this file are LOCAL (per the block prompt §5: no
    imports from other test files). They are intentionally a mirror
    of those in test_task_created_source_quality_step.py, kept
    independent so a refactor of one file does not break the other.

  - Test 3 (resume from 'compiling') constructs a real "resume from
    compiling" state: we force the task into status='compiling' with
    a pre-existing audit chain that already contains
    task.source_quality_assessed AND task.entailment_checked AND
    task.compiling (the state a previous fresh run would have left
    if it crashed between 'compiling' and the compiler's INSERT).
    The resume delivery uses a FRESH event_id + fresh
    idempotency_key so the consumer enters the pipeline orchestrator
    past the EPR short-circuit; the orchestrator sees status='compiling'
    (not 'analyzing'), skips _run_8_3_extract_and_verify entirely,
    and falls through to _run_8_4_compile_and_gate. The 8.8A step
    lives inside _run_8_3_extract_and_verify, so the
    entailment_checked audit count MUST stay at 1.

  - Test 4 monkeypatches the symbol
    ``app.consumers.task_created.run_claim_entailment_checks`` to a
    stub that raises a RuntimeError. The patched binding lives on
    the consumer module because task_created imports the function
    at module load time (``from ..services.claim_entailment_orchestrator
    import run_claim_entailment_checks``), so the consumer's local
...
CONSUMER_NAME = "test_task_created_entailment_step"
...
# ---------------------------------------------------------------------------
# generic helpers (LOCAL to this file)
# ---------------------------------------------------------------------------
def _unique_hex() -> str:
    ...
    project_id = uuid.UUID(
        str(
            conn.execute(
                ...
                {"t": tenant_id, "n": f"cec-step-test-{uuid.uuid4()}"},
            ).first()[0]
        )
    )
...
# ---------------------------------------------------------------------------
# audit / DB inspection helpers (LOCAL)
# ---------------------------------------------------------------------------
...
def _pairs_with_entailment_check(
    conn: Connection, *, task_id: uuid.UUID
) -> set[tuple[uuid.UUID, uuid.UUID]]:
    """Return the set of (claim_ledger_entry_id, evidence_span_id)
    pairs (from THIS task's claim_evidence_links) for which a
    claim_entailment_checks row exists.
    """
    rows = conn.execute(
        text(
            """
            SELECT DISTINCT
                cec.claim_ledger_entry_id AS entry_id,
                cec.evidence_span_id      AS span_id
            FROM claim_entailment_checks cec
            WHERE (cec.claim_ledger_entry_id, cec.evidence_span_id) IN (
                SELECT DISTINCT
                    cel.claim_ledger_entry_id,
                    cel.evidence_span_id
                FROM claim_evidence_links cel
                JOIN logical_claims        lc ON lc.id = cel.claim_logical_id
                WHERE lc.task_id = :tid
                  AND cel.evidence_span_id IS NOT NULL
...
# ===========================================================================
# 1) pipeline emits task.entailment_checked after task.source_quality_assessed
#    and before task.compiling; audit chain verifies
# ===========================================================================
def test_pipeline_emits_task_entailment_checked_after_source_quality_before_compiling():
    """Scenario 1 of the block prompt §5.

    A full fresh-run pipeline must emit task.entailment_checked
    EXACTLY ONCE, with chain_seq strictly between
    task.source_quality_assessed and task.compiling. The audit
    payload must carry status='completed',
    evaluated_target_kind='claim_evidence_pair', and a counts dict
    where pairs_total >= 1 and the per-status counters sum to
    pairs_total.
    """
    ...
    # Build the position map. Each event must be present exactly once
    # on this fresh-run path; if any of them is missing we get a
    # KeyError which fails the test clearly.
    ...
    assert "task.entailment_checked" in positions, chain
    assert "task.compiling" in positions, chain

    # Order invariant: entailment_checked sits strictly between
    # source_quality_assessed and compiling on the task chain, and
    # source_quality_assessed sits strictly after analyzed_partial.
    assert (
    ...
    # Exactly one entailment_checked audit (no duplicate, no resume
    # re-emission).
    with engine.connect() as conn:
        ...
        # Payload shape: status='completed',
        # evaluated_target_kind='claim_evidence_pair', counts is a
        # dict, every required counter is present and non-negative.
        payload = _fetch_audit_payload(
            conn, task_id=task_id, event_type="task.entailment_checked"
        )
        assert payload["status"] == "completed"
        assert payload["evaluated_target_kind"] == "claim_evidence_pair"
        counts = payload["counts"]
        assert isinstance(counts, dict)
        assert counts["status"] == "completed"
        assert counts["pairs_total"] >= 1
        # The completion arithmetic must balance: every pair ends up
        # in exactly one of the per-status buckets.
        bucket_total = (
        ...
        assert bucket_total == counts["pairs_total"]


# ===========================================================================
# 2) pipeline populates claim_entailment_checks for every claim/evidence pair
# ===========================================================================
def test_pipeline_populates_claim_entailment_checks_for_claim_evidence_pairs():
    """Scenario 2 of the block prompt §5.

    For each (claim_ledger_entry_id, evidence_span_id) pair linked to
    the task via claim_evidence_links, a row must exist in
    claim_entailment_checks after the pipeline completes.
    """
    _skip_if_db_unreachable()
    tenant_id, project_id, _user_id, task_id = _setup_task_with_doc()
    ...
    engine = get_engine()
    with engine.connect() as conn:
        expected_pairs = _distinct_pairs_for_task(conn, task_id=task_id)
        actual_pairs = _pairs_with_entailment_check(conn, task_id=task_id)

    assert len(expected_pairs) >= 1, (
        "test corpus must produce at least one claim_evidence_link "
        "with a non-null evidence_span_id"
    )
    # Every linked pair has a corresponding claim_entailment_checks
    # row. We compare as sets so an extra unrelated pair would not
    # cause a failure, but every linked pair MUST be present.
    assert expected_pairs.issubset(actual_pairs), (
        f"pairs missing from claim_entailment_checks: "
        f"{expected_pairs - actual_pairs}"
    )


# ===========================================================================
# 3) resume from 'compiling' does NOT re-emit task.entailment_checked
# ===========================================================================
def test_pipeline_resume_from_compiling_does_not_reemit_entailment_audit():
    """Scenario 3 of the block prompt §5.
    ...
    the 8.8A event task.entailment_checked, and task.compiling —
    exactly as the fresh-run path would have emitted them.
    ...
    ``_run_8_4_compile_and_gate`` to resume the 8.4 stage. The 8.8A
    step lives inside ``_run_8_3_extract_and_verify``, so the
    entailment audit count MUST stay at 1. The pipeline must complete
    successfully and the audit chain must verify.
    """
    _skip_if_db_unreachable()
    tenant_id, project_id, _user_id, task_id = _setup_task_with_doc()
    ...
    with engine.connect() as conn:
        ...
        entailment_audit_before = _count_audit_event(
            conn,
            task_id=task_id,
            event_type="task.entailment_checked",
        )
    ...
    assert entailment_audit_before == 1
    ...
    with engine.connect() as conn:
        # Critical invariant: entailment audit was NOT re-emitted.
        entailment_audit_after = _count_audit_event(
            conn,
            task_id=task_id,
            event_type="task.entailment_checked",
        )
    ...
    assert entailment_audit_after == 1, (
        "task.entailment_checked must NOT be re-emitted on resume "
        "from 'compiling'"
    )
...
# ===========================================================================
# 4) entailment failure is audited but does NOT block 8.4
# ===========================================================================
def test_pipeline_entailment_failure_is_audited_but_does_not_block_8_4(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Scenario 4 of the block prompt §5.

    Monkeypatch the orchestrator entrypoint on the consumer module
    (NOT on the orchestrator module) to raise a RuntimeError. The
    consumer must:

      - return rc == 'processed' (the 8.4 pipeline ran to completion);
      - emit exactly one task.entailment_checked audit row;
      - on that row, status='failed', error_type='RuntimeError',
        counts.error_count == 1, all other counts == 0;
      - leave claim_entailment_checks with NO rows for this task's
        pairs (the SAVEPOINT rolled back any partial insert);
      - still produce a final_gate_report (8.4 completed despite
        8.8A failure);
    ...
    # run_claim_entailment_checks at module load and binds the name
    # locally.
    def _raise_runtime(*_args, **_kwargs):
        raise RuntimeError("simulated claim entailment failure")
    ...
    # The pipeline MUST succeed end-to-end despite the 8.8A failure.
    assert rc == "processed"
    ...
    with engine.connect() as conn:
        ...
        # claim_entailment_checks INSERT from this call. With the
        # orchestrator stubbed to raise BEFORE any insert, the table
        # remains empty for this task's pairs.
        pairs_with_checks = _pairs_with_entailment_check(conn, task_id=task_id)
        assert pairs_with_checks == set(), (
            f"savepoint rollback failed: claim_entailment_checks "
            f"leaked rows for pairs {pairs_with_checks}"
        )
        ...
        # not fail because of the 8.8A error").
        gate_count = int(
            conn.execute(
