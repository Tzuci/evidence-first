...
> **Stato corrente.** Repository al commit **`28cecbe`** ("Add multi-agent mock orchestration runner"). **Punto più recente: sotto-fase ORCH-MULTI-A** (bounded multi-agent **mock** orchestration runner). La sotto-fase **8.8B-REPORT** (commit `af74187`, "Fix report CVE lineage and add realistic flow") **resta tecnicamente conclusa e invariata**, ma non è più il punto più recente del repository. Il repo ora include anche una **orchestration foundation** (schema + mock provider abstraction + single-agent runner + bounded multi-agent runner), **tutta mock-only** e **non integrata** nella pipeline `task.created`, nella UI o nella publication. Vedi `PROJECT_STATE.md` e `ORCH_MULTI_A_IMPLEMENTATION_REPORT.md`.
...
- **Orchestration foundation (mock-only)**: orchestration schema foundation, mock provider abstraction, single-agent mock orchestration runner, bounded multi-agent mock orchestration runner (ORCH-MULTI-A). Tutta mock-only, separata dalla pipeline `task.created`; vedi [`ORCH_MULTI_A_IMPLEMENTATION_REPORT.md`](ORCH_MULTI_A_IMPLEMENTATION_REPORT.md)
...
Incluso (oggi, post-ORCH-MULTI-A):
...
- **Orchestration foundation, mock-only** (ORCH-SCHEMA / ORCH-PROVIDER / ORCH-RUNNER / ORCH-MULTI-A): schema + mock provider abstraction + single-agent runner + bounded multi-agent runner. Funzione pubblica multi-agent: `run_multi_agent_mock_orchestration`. Test finale ORCH-MULTI-A documentato: **17 passed**. Distinzioni da preservare: **provider output ≠ final answer**; **source candidate ≠ evidence**; **provider citation/source ≠ verified evidence**; l'output dell'orchestration runner **non viene pubblicato**; il **Final Answer Gate NON è eseguito** da ORCH-MULTI-A (`publication_status="not_evaluated"`, `gate_report_id=None`). I `source_candidates` sono proposte non verificate, non `evidence_spans` né `claim_evidence_links`; incidono su downstream solo dopo future fasi di source resolution / retrieval / evidence extraction / source verification / claim binding / gate evaluation (nessuna esistente oggi).
...
- **validatore citazione-claim** (8.8B storico, distinto da 8.8B-REPORT): verifica che il claim citi le evidenze giuste, non solo "vicine". Mancante.
- **detector di contraddizioni reale** (8.8C): cross-source contradiction su `contradiction_records`. Mancante.
...
- **UI completa**: nessuna interfaccia utente espone ancora il report aggregato né i gap entailment / source_quality. Mancante. **UI-PRE resta il prossimo blocco del ramo UI; sul ramo orchestration il prossimo blocco consigliato è ORCH-MULTI-B-PRE.**
...
- Retention reale distruttiva (`0012_*` o successiva; `0011_orchestration_schema.sql` è già occupata dalla foundation orchestration).
...
- **Orchestration — ancora esclusi (post ORCH-MULTI-A):** provider reali (oggi solo mock); source resolution / retrieval / verification per l'orchestration; synthesis e publication dall'orchestration; API / UI orchestration; Final Answer Gate integration per l'orchestration runner; evidence extraction e claim binding da `source_candidates`. Esiste oggi **solo** un bounded multi-agent **mock** runner.
...
- Nessun **Verified Web Mode**, **Hybrid Mode**, **consensus engine**, **detector avanzato di contraddizioni**, **critical reviewer**.
...
`make migrate` applica le migration presenti nel repository in ordine, oggi fino a `0011_orchestration_schema.sql`. La retention reale distruttiva resta futura e dovrà usare `0012_*` o successiva. **8.8B-REPORT non ha introdotto nuove migration.**
...
- [`ORCH_MULTI_A_IMPLEMENTATION_REPORT.md`](ORCH_MULTI_A_IMPLEMENTATION_REPORT.md) — implementation report ORCH-MULTI-A (bounded multi-agent mock orchestration runner)
