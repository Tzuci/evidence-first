...
## 13. Prompt successivo per 8.7H-CODE

```
PROMPT PER CLAUDE — Evidence-First MVP-0 — Blocco 8.7H-CODE

Ruolo: senior backend engineer responsabile del progetto. Devi
implementare il realistic flow 8.7H secondo PHASE_8_7H_PRE.md.

Stato di partenza: stesso commit di 8.7H-PRE (post-8.7G-DOC).

File da leggere PRIMA di scrivere:
- PHASE_8_7H_PRE.md (questo è il piano)
- tests/test_phase_8_5_source_loss_flow.py (template bootstrap)
- tests/test_phase_8_6_read_flow.py (template HTTP assertions)
- apps/worker/tests/test_task_created_source_quality_step.py
  (template seed + monkeypatch orchestrator)
- apps/worker/tests/test_final_answer_gate_source_quality.py
  (template scenari Gate)
- apps/api/tests/test_source_quality_read_endpoint.py
  (template HTTP assertions read API 8.7F)
- apps/api/app/routes/tasks.py (per signature POST /tasks)
- apps/api/app/routes/documents.py (per signature upload)
- apps/worker/app/consumers/task_created.py (per shape audit payload
  e signature run_source_quality_assessment)

Obiettivo: creare ESCLUSIVAMENTE il file
tests/test_phase_8_7_source_quality_flow.py con due test functions:

1. test_phase_8_7_source_quality_warning_flow_end_to_end
2. test_phase_8_7_source_quality_block_flow_end_to_end

Seguendo §4, §5, §8 di PHASE_8_7H_PRE.md.

Vincoli operativi:
- Non modificare nessun altro file in 8.7H-CODE.
- Non importare helper da altri test file.
- Tutti gli UUID/hash/idempotency key devono essere rerun-safe.
- Tutte le query SQL devono usare bound params.
- Le asserzioni HTTP devono verificare status_code, shape JSON,
  e contenuti specifici (reason_code, kind dei gap, ecc.).
- L'audit chain deve essere verificata via verify_task_audit_chain.
- Snapshot pre/post su row counts NON è richiesto in questo blocco
  (gli endpoint 8.7F sono già coperti da test_source_quality_read_endpoint.py
  per la read-only invariance).

Test plan minimo (vedi §8.3 PHASE_8_7H_PRE.md per il dettaglio):

Warning flow:
- seed dev tenant + user
- project + doc + task via API HTTP
- FakeRedis cattura task.created
- dispatch.handle_event -> "processed"
- DB: task=published, audit chain ok, sqa con unknown, gate report
  con all_spans_verified_with_warnings, gap source_quality_warning
  per ogni span, published_answers v1
- HTTP: /final-gate-report 200, /published-answer 200,
  /tasks/{id}/source-quality 200 con summary.unknown >= 1,
  /evidence-spans/{es}/source-quality 200

Block flow:
- seed dev tenant + user
- monkeypatch _wapp.consumers.task_created.run_source_quality_assessment
  con stub che inserisce sqa unsuitable
- project + doc + task via API HTTP
- FakeRedis cattura task.created
- dispatch.handle_event -> "processed"
- DB: task=analyzed_partial, audit chain ok, sqa con unsuitable,
  gate report con source_quality_block, gap source_quality_block,
  NO published_answers, NO source_quality_warning
- HTTP: /final-gate-report 200 con source_quality_block,
  /published-answer 404 con details.resource='published_answers',
  /tasks/{id}/source-quality 200 con summary.unsuitable >= 1

Al termine del blocco, FILE_COMPLETATI deve elencare solo:
- tests/test_phase_8_7_source_quality_flow.py

Non aggiornare PROJECT_STATE.md, PHASE_8_7_PLAN.md, README.md in
8.7H-CODE: quelli sono compito di 8.7H-DOC (blocco successivo
opzionale).

Eseguire `pytest tests/test_phase_8_7_source_quality_flow.py -v`
e verificare che entrambi i test passino.

Eseguire `pytest tests/` per verificare nessuna regressione sui
flow 8.5/8.6 (co-residenza alias _wapp).

Eseguire `pytest apps/worker/tests/ apps/api/tests/` per verificare
nessuna regressione sui test unit pre-esistenti.
```
